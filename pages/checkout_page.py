...
header1, header2, header3 = st.columns(
    CHECKOUT_LAYOUT, vertical_alignment='center',)
...
if not cart_items:
    st.html('<h3 style="text-align:center; opacity:0.8;">Cart is currently empty<h3/>')

for item_name, qty in cart_items:
    item_price = f'${sku_data[item_name]['price']:.2f}'
    with st.container():
        item_name_col, item_total_price_col, item_qty_col = st.columns(
            CHECKOUT_LAYOUT, vertical_alignment='center',)
        with item_name_col:
            st.write(item_name)
        ...
        with item_qty_col:
            ...
            col1.button('\-', key=f'{item_name}_minus',
                        on_click=mylibrary.subtract_cart_qty, args=(item_name,))
            col2.html(
                f'<p style="width: auto; margin:auto; text-align: center;">{qty}</p>')
            col3.button('\+', key=f'{item_name}_plus',
                        on_click=mylibrary.add_cart_qty, args=(item_name,))
...
with st.container(horizontal_alignment='right'):
    ...
    st.html(
        f'<h1 style="text-align:right; margin-top:0">${total_price:.2f}<h1>')
