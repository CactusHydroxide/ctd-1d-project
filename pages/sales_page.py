...
revenue_columns[1].metric(label='Average Transaction Value',
                          value=f'${total_revenue/len(sales_data):.2f}')
revenue_columns[2].metric(label='Latest Transaction',
                          value=f'${sales_data[-1]['price']:.2f}')
...
with st.container(height=700):
    for idx, transactions in enumerate(sales_data[::-1]):
        ...
        st.html(
            f'<h3 style="margin-bottom:0">Transaction ID: 100000{len(sales_data)-idx}</h3>')
        st.html(
            f'<h3 style="margin-top:0">{transactions['timestamp'].strftime("%d %b %Y %H:%M:%S")}</h3>')
